...
@Component
class TimerTargetTimeUpdateListener(
    ...
    fun init() {
        ...
        val topic = "$env:${timerRedisProps.pubSub.targetTimeUpdatedChannel}"
    ...
    override fun onMessage(
        ...
        val msg =
            runCatching { objectMapper.readValue(body, TimerUpdateTargetTimeMessage::class.java) }
                .onFailure { log.error("Message parsing failed body: {}", body) }
                .getOrNull() ?: return

        log.debug("onMessage: {}", msg)

        timerUseCase.updateTargetTime(
            msg.timerId,
            msg.payload.updatedAt,
            msg.payload.serverTime,
            msg.payload.newTargetTime,
        )
    }

    companion object {
        private val log = LoggerFactory.getLogger(TimerTargetTimeUpdateListener::class.java)
    }
}
